    ...
    if (type === 'collegeImage') {
      if (files.length > 0) {
        if (!validTypes.includes(files[0].type)) {
          setMessage({ type: 'error', text: 'Only JPEG or PNG images are allowed' });
          return;
        }
      ...
      // Revoke previous preview URLs for removed images
      formData.photos.forEach(photo => URL.revokeObjectURL(photo.preview));
    ...
    try {
      ...
      Object.keys(formData).forEach(key => {
        if (key === 'collegeImage' && formData[key]) {
          ...
          submitData.append('streams', JSON.stringify(formData.streams));
        } else if (key === 'subjects') {
          submitData.append('subjects', JSON.stringify(formData.subjects));
        } else if (formData[key] !== null && formData[key] !== undefined && key !== 'collegeImagePreview') {
      ...
      // Call the API
      const response = await puCollegeApi.addCollege(submitData);
      
      setMessage({
        type: 'success',
        text: response.message || 'PU College added successfully!'
      });
      ...
      console.error('Error adding PU college:', error.response?.data, error.message);
      setMessage({
        type: 'error',
        text: error.response?.data?.errors?.join(', ') || 
             error.response?.data?.message || 
             error.message || 
             'Failed to add PU college. Please try again.'
      });
    } finally {
      setLoading(false);
    }
      ...
      >
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8 border border-orange-100">
          ...
          <motion.button
            ...
            aria-label="Submit PU College Details"
          >
            {loading ? 'Submitting...' : 'Submit PU College Details'}
          </motion.button>
        </div>
      </motion.div>
    </div>
