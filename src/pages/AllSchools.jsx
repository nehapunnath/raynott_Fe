...
function AllSchools() {
    ...
    return [
      {
        ...
        items: Object.values(schools).map((school) => {
          ...
          return {
            ...
            location: school.address || school.city || 'Unknown Location',
            fees: school.totalAnnualFee
              ? `₹${school.totalAnnualFee.toLocaleString()}`
              : 'Fees not available',
  ...
  const fetchFilteredSchools = async (filterParams) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || err.message || 'Failed to fetch filtered schools');
      setSchoolsData([]);
      setTotalSchools(0);
    } finally {
      setLoading(false);
  ...
  const applyFilters = () => {
    const filterParams = {
      ...
      board: filters.board.length > 0 ? filters.board[0] : undefined,
      schoolType: filters.schoolType.length > 0 ? filters.schoolType[0] : undefined,
            ...
            <div key={index} className="mb-12">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                {section.items.map((item) => (
                  <Link to={`/school-details/${item.id}`} key={item.id} className="group">
                    <motion.div
